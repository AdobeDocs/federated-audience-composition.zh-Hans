...
数据模型是一组架构、受众以及它们之间的链接。 您可以使用模型将数据库的数据合并到受众中。
...
2. 填写给定的表单以定义链接。
   ...
   * **1-1**：源表的一个项最多对应目标表的一个项。
   ...
   ![加号按钮突出显示，允许您为模型创建多个连接链接。](assets/models/multi-join.png){zoomable="yes"}
...
2. 选择Source联接旁边的![](assets/models/do-not-localize/Smock_AddCircle_18_N.svg)按钮，然后拖动箭头并引导至Target联接以建立连接。
