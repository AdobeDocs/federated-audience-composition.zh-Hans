...
联合受众组合支持&#x200B;**三个**&#x200B;类别的用例：受众创建、受众扩充和客户配置文件扩充。

* 受众创建：您可以从Data Warehouse创建受众，并将这些受众联合到Experience Platform中，以便通过营销人员友好的拖放用户界面在Real-Time CDP或Journey Optimizer中使用。 因此，您可以查询数据仓库，而无需复制敏感的基础数据或复制现有数据。
   * **示例：**&#x200B;使用仓库中的历史交易数据创建具有高额过去的购买者的受众，而无需将这些交易复制到Experience Platform。

* 受众扩充：通过使用数据仓库中的其他数据集并使用此信息叠加受众，您可以在Experience Platform中向现有受众添加更多详细信息 — 所有这些操作都无需将基础数据复制到Experience Platform。 通过受众扩充，您可以使用扩充的受众提供改进的个性化。
   * **示例：**&#x200B;通过高价值过去购买者的联合受众构成受众，丰富了购物车放弃者的Experience Platform受众，以投放目标选件。

* 配置文件扩充：您可以从数据仓库中选择单个客户属性以增强Experience Platform配置文件。 通过向这些用户档案中添加联合数据，您可以更好地提供由入站客户信号触发的即时体验。
   * **示例：**&#x200B;使用联合受众的信息扩充了Experience Platform配置文件。 现在，您可以向属于高价值过去购买者联合受众的网站访客进行营销，向其提供由其网站行为触发的定向优惠。

![图表](assets/fac-use-cases.png){zoomable="yes"}{width="75%" align="center"}

有关联合受众组合用例的更多信息，请阅读[联合受众组合白皮书](https://business.adobe.com/resources/sdk/flexibly-access-enterprise-data-with-federated-audience-composition.html)。
...
>[!NOTE]
>
>执行合成后，生成的受众将作为外部受众保存在Adobe Experience Platform中，并可用于Adobe Real-Time Customer Data Platform和/或Adobe Journey Optimizer中。 它可在&#x200B;**受众**&#x200B;菜单中访问。[了解详情](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/segmentation/ui/audience-portal){target="_blank"}
