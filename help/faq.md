...
为了使用联合受众构成，必须将每个用户添加到为每个沙盒创建的特定轮廓中。若要了解更多信息，请参阅[访问联合受众构成](./start/access-prerequisites.md)页面。
...
有关连接到Data Warehouse的详细信息，请阅读[连接概述](./connections/home.md)。
...
是的，同一构成中可以查询多个仓库，并且可以组合多个来源的数据。通常，每个[构成活动](./compositions/activities.md)（查询、扩充、拆分等）根据活动配置、目标数据库（可以有多种联合数据访问的情况）以及执行结果的一个或多个工作表的输出执行一条或多条 SQL 语句。这些工作表用作连续活动的输入。
...
## 如果合并来自多个源的数据，我们如何连接这些数据？ 我们使用身份标识服务吗？
